...
class Fail(Result[R]):
    ...
    def fmap(self, f: Callable[[R], S]) -> Result[S]:
        return Fail(self.view, self.ctx, self.msg)

from typing_extensions import Protocol

class ParserF(Protocol[R]):
    def __call__(self, i: memoryview) -> Result[R]: ...

@dataclass
class Partial(Result[R]):
    run_partial: ParserF[R]

    def fmap(self, f: Callable[[R], S]) -> Result[S]:
        def run(i: memoryview) -> Result[S]:
            return self.run_partial(i).fmap(f)
        return Partial(run)
...
class Done(Result[R]):
    ...
    def fmap(self, f: Callable[[R], S]) -> Result[S]:
        return Done(self.view, f(self.result))

@dataclass
class Parser(Generic[R]):
    run_parser: ParserF[R]
